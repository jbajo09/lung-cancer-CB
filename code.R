...
dataPlot(knn_tri_4A_1a_2$cfMats[[9]]$table,labels_tri_rna  ,mode = "confusionMatrix",toPNG = FALSE, toPDF = FALSE)
...
#triclase
genes_tri <- c('KRT15','TFRC','S100A2','COL17A1','GPX2','FERMT1','SPRR1B','ZNF750','MELK')
...
dataPlot(knn_tri_4B_1_2$cfMats[[10]]$table,labels_tri_mic_1  ,mode = "confusionMatrix",toPNG = FALSE, toPDF = FALSE)
...
###     VALIDANDO EN DATASET RNA-SEQ DIFERENTE                     ###
...
pa <- function(data){
  ...
  axis(1, at=seq_len(ncol(x)), labels=FALSE, cex.axis=5, font = 2)
  title(xlab='Prediction', line=4.5, cex = 4,cex.lab=2.5)
  text(seq_len(ncol(x)), par("usr")[3] - 0.1, labels = colnames(x), pos = 1 ,font = 2, xpd = TRUE,cex=2.5)
  
  axis(2, at=ncol(x):1, labels=colnames(x), las=1, cex.axis=2.5,font = 2)
  title(ylab='Reference', line=12.5, cex=4, cex.lab=2.5)
  abline(h = 0:ncol(x) + 0.5, col = 'gray')
  abline(v = 0:ncol(x) + 0.5, col = 'gray')
  text(seq_len(ncol(x)), rep(ncol(x):1, each=ncol(x)),
       labels = c(x.orig),cex=2.5, font=2)
  box(lwd=2)
  par(opar) # reset par
  
  # add in the specifics
  plot(c(100, 0),c(0, 50),  type = "n", xlab="", ylab="", main = "DETAILS", xaxt='n', yaxt='n', cex.main=2 )
  text(20, 35, names(res$overall[1]), cex=2.5, font=2)
  text(20, 15, round(as.numeric(res$overall[1])*100, 3), cex=2.5)
  
  if(ncol(x) > 2){
    
    text(40, 35, colnames(res$byClass)[7], cex=2.5, font=2)
    res$byClass[is.na(res$byClass[,7]),7] <- 0
    text(40, 15, round(mean(as.numeric(res$byClass[,7]))*100, 3), cex=2.5)
    text(60, 35, colnames(res$byClass)[1], cex=2.5, font=2)
    text(60, 15, round(mean(as.numeric(res$byClass[,1]))*100, 3), cex=2.5)
    text(80, 35, colnames(res$byClass)[2], cex=2.5, font=2)
    text(80, 15, round(mean(as.numeric(res$byClass[,2]))*100, 3), cex=2.5)
    
  }else{
    
    text(50, 35, names(res$byClass)[1], cex=2.5, font=2)
    text(50, 15, round(mean(as.numeric(res$byClass[1]))*100, 3), cex=2.5)
    text(80, 35, names(res$byClass)[2], cex=2.5, font=2)
    text(80, 15, round(mean(as.numeric(res$byClass[2]))*100, 3), cex=2.5)
...
pa(knn_SCC_mic$cfMats[[4]]$table)
pa(knn_SCC_rna$cfMats[[4]]$table)
pa(knn_tri_4A_1a_2$cfMats[[9]]$table)
pa(knn_tri_4B_1_2$cfMats[[9]]$table)
