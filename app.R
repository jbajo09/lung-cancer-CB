...
labels_mic <- as.vector(t(labels_mic))
labels_rna <- as.vector(t(labels_rna))
              ...
              the statistical significance of the results.",
                        ...
                        tabItem(tabName = "validation",
                                ...
                                sliderInput(inputId = "numero_genes_validation", label = "Select the number of genes to use:",
                                            value = 8, min = 1, max = 50, step = 1, width = "50%"),
                        ...
                        tabItem(tabName = "codigo",
                                h1("Code"),
                                tags$h4(
                                  "In ", tags$a(href = "https://github.com/jbajo09/lung-cancer-CB.git", "this repository"),
...
server <- function(input, output){
  ...
  observeEvent(input$boton_genes, {
    ...
    DEGs_CV <- DEGsExtraction(t(DEGsMatrix_mic), as.factor(labels_mic), lfc=as.numeric(input$LFC), cov = as.numeric(input$COV), pvalue = as.numeric(input$pvalue), number = Inf, CV = TRUE, numFolds=10)
  ...
  observeEvent(input$boton_model_validation, {
    ...
    set.seed(333)
    indices_rna <- createDataPartition(labels_rna, p = 0.80, list = FALSE,  times = 1)
    particion.entrenamiento.rna <- DEGsMatrix_rna[indices_rna,]
    particion.test <- DEGsMatrix_rna[-indices_rna,]
